	...
	<div on:mouseenter={mouseEnter} on:mouseleave={mouseLeave} class="relative z-10 flex gap-4 pt-4 pb-4 px-4 {flip ? 'flex-row' : 'flex-row-reverse'}">
		...
		<div class="flex flex-col justify-center w-1/2">
			<div class="flex flex-row justify-center px-8">
				<Link document={linkSrc} activeClass='' classes="h2 w-full flex justify-between bg-white shadow-lg hover:shadow-2xl transition-all translate-y-0 duration-500	hover:-translate-y-[2px]	 rounded-full px-8 py-4" >
